...
if response.status == 200 do
  State.set("debug",response.body) 
  item = Json.parse (response.body)
    
  State.set("debug2",item.row)
...
async function run() {
    return new Promise(async function (resolve, reject) {
        try {
            ...
            const contractSeed = "denise.valette.contract.5" //randomBytes(32)
            const contractAddress = Crypto.deriveAddress(contractSeed)

            const callerSeed = "denise.valette.caller.0"  //randomBytes(32)
